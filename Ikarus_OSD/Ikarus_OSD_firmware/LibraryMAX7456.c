...
void locateAt(unsigned char fila, unsigned char col)
{
	int dir;
...
void writeAtChr(int fila, int col, char c)
{
	...
	if(fila<filas && fila>=0 && col<MAXCOLS)
	{
		locateAt(fila, col);
		waitSync();
		sendValue(0x7,c);	//car
...
void printAtStr2(int fila, int col, char cad[], char len)
{
	...
	if(fila<filas && fila>=0 && col<MAXCOLS)
	{
		...
		sendValue(0x4,readValue(0x84)|0x1);			// Modo autoincrementar	

		for(i=0;i<len && cad[i]!=0 && col< MAXCOLS; i++, col++)
		{
			//locateAt(fila, col);
			waitSync();
			NSS=0; //Transferimos los 3 bytes.
			sendByte(convertChr(cad[i]));
			NSS=1;	
...
void printAtStr(int fila, int col, char cad[])
{
	printAtStr2(fila, col, cad, strlen(cad));
}
